...
public interface EmployeeMapper {
     ...
     * @return
     */
    @Select("select * from campus_delivery.employee where username = #{username}")
    Employee getByUsername(String username);

    @AutoFill(value = OperationType.INSERT)
    @Insert("insert into campus_delivery.employee (name, username, password, phone, sex, id_number, status,create_time, update_time, create_user, update_user) " +
    ...
    * */
     ...
     * 根据id查询员工
     * */
    @Select("select * from campus_delivery.employee where id = #{id}")
